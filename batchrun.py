...
for i in range(len(combinations)):
    ...
    for j in range(steps + 1):
        model = ABM_Final2406_new.SocialNetwork(n_agents, prob, concentration, w_popularity, w_proximity, w_similarity, mu, temp)
    ...
    df_results.to_csv(f"w_pop{w_popularity}w_prox{w_proximity}w_sim{w_similarity}.csv", index=False)
